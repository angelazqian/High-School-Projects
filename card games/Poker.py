...
while play:
  ...
  if state == "betting":
    ...
    compbet = math.ceil(random.random() * random.random() * compchips)
    compbet = math.ceil(compbet * (1+(playerfinal-compfinal)/10))
    ...
    if gamecount%2 == 1:
        ...
        if notFold:
            if (random.randint(1, 10) == 1 and compchips != 1) or compbet < playerbet:
        ...
        if (random.randint(1, 10) == 1 and compchips != 1):
            print ("Your opponent goes first and folds! You gain 1 token and move to the next hand.")
            playerchips += 1 
            compchips -= 1 
            notFold = False 
        else:
            print ("The opponent starts and bets ", compbet, " tokens. If you would like to fold and lose 1 token to move on to the next hand, please type '0'. If you would like to make a bet, please type in the number of tokens you are wagering. You must match the computer's bet, and you may not bet more tokens than you have.")
            playerbet = int(input("How much do you bet? "))
            if playerbet == 0 or playerbet < compbet:
                playerchips -= 1 
                compchips += 1 
                notFold = False
                print ("Moving on to next round")
